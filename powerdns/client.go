...
func (client *Client) GetZone(name string) (ZoneInfo, error) {
	...
	_, err = client.do(req, &zoneInfo)
	if err != nil {
		return ZoneInfo{}, fmt.Errorf("Error getting zone: %s, reason: %q", name, err)
	}
...
func (client *Client) ZoneExists(name string) (bool, error) {
	...
	if err != nil && (resp == nil || resp.StatusCode != http.StatusNotFound) {
		return false, fmt.Errorf("Error getting zone: %s, reason: %q", name, err)
	}
...
func (client *Client) CreateZone(zoneInfo ZoneInfo) (ZoneInfo, error) {
	...
	_, err = client.do(req, &createdZoneInfo)
	if err != nil {
		return ZoneInfo{}, fmt.Errorf("Error creating zone: %s, reason: %q", zoneInfo.Name, err)
	}
...
func (client *Client) UpdateZone(name string, zoneInfo ZoneInfo) error {
	...
	if err != nil {
		return fmt.Errorf("Error updating zone: %s, reason: %q", zoneInfo.Name, err)
	}
...
func (client *Client) DeleteZone(name string) error {
	...
	_, err = client.do(req, nil)
	if err != nil {
		return fmt.Errorf("Error deleting zone: %s, reason: %q", name, err)
	}
...
func (client *Client) ReplaceRecordSet(zone string, rrSet ResourceRecordSet) (string, error) {
	...
	_, err = client.do(req, nil)
	if err != nil {
		return "", fmt.Errorf("Error creating record set: %s, reason: %q", rrSet.ID(), err)
	}
...
func (client *Client) DeleteRecordSet(zone string, name string, tpe string) error {
	...
	_, err = client.do(req, nil)
	if err != nil {
		return fmt.Errorf("Error deleting record: %s %s, reason: %q", name, tpe, err)
	}
...
func (client *Client) setServerVersion() error {
	...
	resp, err := client.do(req, &serverInfo)
	...
	return fmt.Errorf("Unable to get server version. Reason: %q", err)
}
...
func (client *Client) do(req *http.Request, v interface{}) (*http.Response, error) {
	...
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		...
		if resp.Header.Get("Content-Type") == "application/json" {
			...
			apiError.ErrorMsg = string(messageBytes)
		}

		if apiError.ErrorMsg != "" {
			return resp, fmt.Errorf("Error message: %s. Status code: %d", apiError.ErrorMsg, resp.StatusCode)
		}
...
func (client *Client) getZoneInfo(zone string) (*ZoneInfo, error) {
	...
	_, err = client.do(req, &zoneInfo)
	if err != nil {
		return nil, fmt.Errorf("Error getting zone info: %s, reason: %q", zone, err)
	}
